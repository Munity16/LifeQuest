import { ArrowRight, CheckCircle2, Clock3, ScrollText, Swords, Zap } from "lucide-react";
import Link from "next/link";
import type { QuestView } from "@/lib/types";

export function QuestFocusBoard({ readyQuests, completedCount, totalCount }: { readyQuests: QuestView[]; completedCount: number; totalCount: number }) {
  ...
  return (
    <section className="quest-focus-board" aria-labelledby="quest-board-title">
      <div className="quest-board-heading">
        <div><span className="eyebrow"><ScrollText size={14} /> Quest board</span><h2 id="quest-board-title">Choose today&apos;s mission</h2></div>
      ...
      {recommended ? (
        <div className="quest-board-grid">
          <article className="quest-board-featured">
            ...
            <Link href={`/campaign/${recommended.campaignId}/quest/${recommended.id}`}>Begin mission <ArrowRight size={16} /></Link>
          </article>

          <div className="quest-board-side">
            <div className="quest-board-progress"><CheckCircle2 size={19} /><div><strong>{completedCount} / {totalCount} cleared</strong><span>Every victory weakens the nemesis.</span></div></div>
